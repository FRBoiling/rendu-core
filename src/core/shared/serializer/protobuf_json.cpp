...
private:
  ...
  void
  WriteRepeatedMessageField(google::protobuf::Message const &value, google::protobuf::FieldDescriptor const *field);
...
bool Deserializer::Key(const Ch *str, rapidjson::SizeType /*length*/, bool /*copy*/) {
  ...
  if (!field) {
    _errors.push_back(
        rendu::StringFormat("Message %s has no field %s.", _objectState.top()->GetTypeName().c_str(), str));
    return false;
  }
...
bool Deserializer::Uint(uint32 i) {
  ...
  switch (field->cpp_type()) {
    ...
    default:
      _errors.push_back(rendu::StringFormat("Expected field type to be uint32 or string but got %s instead.",
...
bool Deserializer::Double(double d) {
  ...
  switch (field->cpp_type()) {
    ...
    case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
      ...
      break;
    default:
      _errors.push_back(rendu::StringFormat("Expected field type to be float or double but got %s instead.",
                                            _state.top()->cpp_type_name()));
      return false;
  }
...
bool Deserializer::String(const Ch *str, rapidjson::SizeType /*length*/, bool /*copy*/) {
  ...
  switch (field->cpp_type()) {
    case google::protobuf::FieldDescriptor::CPPTYPE_ENUM: {
      ...
      if (!enumValue) {
        _errors.push_back(
            rendu::StringFormat("Field %s enum %s does not have a value named %s.", field->full_name().c_str(),
                                field->enum_type()->full_name().c_str(), str));
        return false;
      }
    ...
    case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
      ...
      break;
    default:
      _errors.push_back(rendu::StringFormat("Expected field type to be string or enum but got %s instead.",
                                            _state.top()->cpp_type_name()));
      return false;
  }
...
bool Deserializer::StartObject() {
  ...
  if (!_state.empty()) {
    if (_state.top()->cpp_type() != google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE) {
      _errors.push_back(
          rendu::StringFormat("Expected field %s to be a message but got %s instead.", _state.top()->cpp_type_name()));
      return false;
    }
...
bool Deserializer::StartArray() {
  ...
  if (_state.top()->is_repeated() ^ (_state.top()->type() != google::protobuf::FieldDescriptor::TYPE_BYTES)) {
    _errors.push_back(
        rendu::StringFormat("Expected field %s type to be exactly an array OR bytes but it was both or none.",
...
bool Deserializer::CheckType(google::protobuf::FieldDescriptor::CppType expectedType) {
  if (_state.top()->cpp_type() != expectedType) {
    _errors.push_back(rendu::StringFormat("Expected field %s type to be %s but got %s instead.",
                                          _state.top()->full_name().c_str(),
                                          google::protobuf::FieldDescriptor::CppTypeName(expectedType),
                                          _state.top()->cpp_type_name()));
    return false;
  }
