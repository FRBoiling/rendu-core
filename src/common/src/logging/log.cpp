...
BEGIN_NAMESPACE_COMMON
    ...
    void Log::CreateAppenderFromConfigLine(std::string const& appenderName, std::string const& options)
    {
        ...
        AppenderType type = AppenderType(Rendu::StringTo<uint8>(tokens[0]).value_or(APPENDER_INVALID));
        LogLevel level = LogLevel(Rendu::StringTo<uint8>(tokens[1]).value_or(LOG_LEVEL_INVALID));
        ...
        if (size > 2)
        {
            if (Optional<uint8> flagsVal = Rendu::StringTo<uint8>(tokens[2]))
    ...
    void Log::CreateLoggerFromConfigLine(std::string const& loggerName, std::string const& options)
    {
        ...
        level = LogLevel(StringTo<uint8>(tokens[0]).value_or(LOG_LEVEL_INVALID));
        if (level > NUM_ENABLED_LOG_LEVELS)
        {
                             ...
                             Utils::FormatStringView messageFormat, Utils::FormatArgs messageFormatArgs) const noexcept
    {
        ...
        {
            LogMessage msg(level, filter, Utils::StringVFormat(messageFormat, std::move(messageFormatArgs)));
            logger->write(&msg);
        }
    }

    void Log::OutCommandImpl(uint32 account, Utils::FormatStringView messageFormat,
                             Utils::FormatArgs messageFormatArgs) const noexcept
    {
        ...
        {
            LogMessage msg(LOG_LEVEL_INFO, "commands.gm",
                           Utils::StringVFormat(messageFormat, std::move(messageFormatArgs)), ToString(account));
            logger->write(&msg);
        }
    }
    ...
    void Log::OutCharDump(std::string const& str, uint32 accountId, uint64 guid, std::string const& name) const noexcept
    {
        ...
        std::string ss = Utils::StringFormat("== START DUMP == (account: {} guid: {} name: {})\n{}\n== END DUMP ==\n",
                                             accountId, guid, name, str);
        std::string param = Utils::StringFormat("{}_{}", guid, name);
