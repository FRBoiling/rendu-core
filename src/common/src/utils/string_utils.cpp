...
#include <Windows.h>
        ...
        void VerifyOsVersion()
        {
            ...
            if (!isWindowsBuildGreaterOrEqual(RENDU_REQUIRED_WINDOWS_BUILD))
            {
                OSVERSIONINFOEX osvi = {sizeof(osvi), 0, 0, 0, 0, {0}, 0, 0, 0, 0};
                GetVersionEx((LPOSVERSIONINFO) & osvi);
                ABORT_MSG(
                    "RenduCore requires Windows 10 19H1 (1903) or Windows Server 2019 (1903) - require build number 10.0.%d but found %d.%d.%d",
                    RENDU_REQUIRED_WINDOWS_BUILD, osvi.dwMajorVersion, osvi.dwMinorVersion, osvi.dwBuildNumber);
            }
#endif
        }


        // 使用指定的分隔符将字符串分割成子字符串视图
        // sep: 分隔符，keepEmpty: 是否保留空字符串
        ...
        Optional<int64> MoneyStringToMoney(std::string const& moneyString)
        {
            ...
            for (std::string_view token : Tokenize(moneyString, ' ', false))
            {
                uint32 unit;
                ...
                Optional<uint64> amount = StringTo<uint32>(token.substr(0, token.length() - 1));
                if (amount)
                    money += (unit * *amount);
                else
                    return std::nullopt;
...
case 0x00FA: // LATIN SMALL LETTER U WITH ACUTE
        ...
        std::string ByteArrayToHexStr(uint8 const* bytes, size_t arrayLen, bool reverse /* = false */)
        {
            ...
            for (int32 i = init; i != end; i += op)
                inserter = StringFormatTo(inserter, "{:02X}", bytes[i]);
