...
//    DateTime::DateTime() : m_timeZ(date::current_zone()->name()), m_timeT(0) {
    ...
    DateTime::DateTime(std::int64_t timeS, std::string timeZ) : m_timeStamp(timeS), m_timeZone(std::move(timeZ)) {
    }
    ...
    std::int64_t DateTime::GetTimeStamp() const {
      return m_timeStamp;
    }
...
//      std::chrono::system_clock::time_point timePoint = std::chrono::system_clock::from_time_t(m_timeStamp);
    ...
    DateTime DateTime::Now(std::string timeZ) {
