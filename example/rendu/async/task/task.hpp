...
#ifndef RENDU_ASYNC_TASK_HPP
#define RENDU_ASYNC_TASK_HPP

#include "broken_promise.hpp"
#include "detail/remove_rvalue_reference.hpp"
#include "task/detail/awaitable_transform.hpp"
...
namespace detail {
  ...
  public:
    static_assert(alignof(T) <= alignof(::max_align_t),
                  "rendu async doesn't allow Task with over aligned object");
...
template<typename AWAITABLE>
auto MakeTask(AWAITABLE awaitable)
    -> Task<detail::remove_rvalue_reference_t<typename detail::AwaitableTraits<AWAITABLE>::await_result_t>> {
