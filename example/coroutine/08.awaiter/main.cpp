...
//
//Task<int,LooperExecutor> DoSomethingAsync(int v) {
//  int value = v;
//  debug("start");
//  using namespace std::chrono_literals;
//  // 模拟一个耗时 3 秒的异步操作
//  co_await SleepAwaiter(3s);
...
//}
//
//Task<int,LooperExecutor> Run(int i) {
//  int value = i;
//  debug("start");
//  auto task = DoSomethingAsync(value);// 调用异步方法，返回一个 Task
//  debug("DoSomethingAsync value is " +std::to_string(value) );
//  auto result = co_await task;// 等待 Task 完成，并获取结果
//  debug(" end,result is " + std::to_string(result));
//  co_return 0;
//}
