#**********************************
#  Created by boil on 2022/10/05.
#**********************************
set(target-name libprotobuf)
...
add_library(${target-name}  ${protobuf_SHARED_OR_STATIC}
  ${libprotobuf_lite_files} ${libprotobuf_files} ${libprotobuf_includes})
if(protobuf_HAVE_LD_VERSION_SCRIPT)
  if(${CMAKE_VERSION} VERSION_GREATER 3.13 OR ${CMAKE_VERSION} VERSION_EQUAL 3.13)
    target_link_options(${target-name}  PRIVATE -Wl,--version-script=${protobuf_source_dir}/src/${target-name} .map)
  elseif(protobuf_BUILD_SHARED_LIBS)
    target_link_libraries(${target-name}  PRIVATE -Wl,--version-script=${protobuf_source_dir}/src/${target-name} .map)
  endif()
  set_target_properties(${target-name}  PROPERTIES
    LINK_DEPENDS ${protobuf_source_dir}/src/${target-name} .map)
endif()
target_link_libraries(${target-name}  PRIVATE ${CMAKE_THREAD_LIBS_INIT})
if(protobuf_WITH_ZLIB)
  target_link_libraries(${target-name}  PRIVATE ${ZLIB_LIBRARIES})
endif()
if(protobuf_LINK_LIBATOMIC)
  target_link_libraries(${target-name}  PRIVATE atomic)
endif()
if(${CMAKE_SYSTEM_NAME} STREQUAL "Android")
  target_link_libraries(${target-name}  PRIVATE log)
endif()
target_include_directories(${target-name}  PUBLIC ${protobuf_source_dir}/src)
...
add_library(protobuf::${target-name}  ALIAS ${target-name} )
