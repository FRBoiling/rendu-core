...
ASSERT_EQ(set.current(traits_type::construct(traits_type::page_size, 1)), traits_type::to_version(rendu::tombstone));
...
ASSERT_DEATH(set.bump(traits_type::construct(traits_type::to_entity(rendu::entity{3}), traits_type::to_version(rendu::tombstone))), "");
}
...
ASSERT_THROW(set.push(std::begin(entities), std::end(entities)), test::throwing_allocator<rendu::entity>::exception_type);
ASSERT_EQ(set.extent(), 2 * traits_type::page_size);
ASSERT_TRUE(set.contains(rendu::entity{0}));
