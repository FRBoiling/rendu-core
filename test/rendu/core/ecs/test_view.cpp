...
TEST(SingleComponentView, Each) {
  ...
  view.each([expected = 1](auto rendu, int &value) mutable {
    ASSERT_EQ(static_cast<int>(rendu::to_integral(rendu)), expected);
  ...
  for(auto [rendu, value]: view.each()) {
    ASSERT_EQ(static_cast<int>(rendu::to_integral(rendu)), value);
  }
}
...
TEST(SingleComponentView, FrontBack) {
  ...
  ASSERT_EQ(view.front(), static_cast<rendu::entity>(rendu::null));
  ASSERT_EQ(view.back(), static_cast<rendu::entity>(rendu::null));
...
TEST(MultiComponentView, Each) {
  ...
  for(auto [rendu, ivalue, cvalue]: registry.view<int, char>().each()) {
    ASSERT_EQ(static_cast<int>(rendu::to_integral(rendu)), ivalue);
    ASSERT_EQ(static_cast<char>(rendu::to_integral(rendu)), cvalue);
  }
}
...
TEST(MultiComponentView, SameComponrenduypes) {
  rendu::registry registry;
