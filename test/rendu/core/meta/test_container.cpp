...
TEST_F(MetaContainer, InvalidContainer) {
  ASSERT_FALSE(rendu::meta_any{42}.as_sequence_container());
  ASSERT_FALSE(rendu::meta_any{42}.as_associative_container());

  ASSERT_FALSE((rendu::meta_any{std::map<int, char>{}}.as_sequence_container()));
  ASSERT_FALSE(rendu::meta_any{std::vector<int>{}}.as_associative_container());
}
...
TEST_F(MetaContainer, StdVector) {
  ...
  ASSERT_TRUE(view);
  ASSERT_EQ(view.value_type(), rendu::resolve<int>());
...
TEST_F(MetaContainer, StdMap) {
  ...
  ASSERT_EQ(view.key_type(), rendu::resolve<int>());
  ASSERT_EQ(view.mapped_type(), rendu::resolve<char>());
  ASSERT_EQ(view.value_type(), (rendu::resolve<std::pair<const int, char>>()));
...
TEST_F(MetaContainer, StdSet) {
  ...
  ASSERT_EQ(view.key_type(), rendu::resolve<int>());
  ASSERT_EQ(view.mapped_type(), rendu::meta_type{});
  ASSERT_EQ(view.value_type(), rendu::resolve<int>());
...
TEST_F(MetaContainer, StdVectorBool) {
  ...
  ASSERT_TRUE(view);
  ASSERT_EQ(view.value_type(), rendu::resolve<bool>());
