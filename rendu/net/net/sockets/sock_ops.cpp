...
int BindSock6(int fd, uint16_t port, const char *ifr_ip) {
  ...
  if (bind(fd, (struct sockaddr *) &addr, sizeof(addr)) == -1) {
    RD_WARN("Bind socket failed:{}", GetErrorMsg(true))
    return -1;
  }
  return 0;
}
...
int BindSock(int fd, uint16_t port, const char *ifr_ip, int family) {
  switch (family) {
    case AF_INET:return BindSock4(fd, port, ifr_ip);
    case AF_INET6:return BindSock6(fd, port, ifr_ip);
    default:return -1;
  }
}
...
SOCKET Bind(uint16_t port, const char *ifr_ip, int family) {
  ...
  switch (family) {
    case AF_INET:return BindSock4(sfd, port, ifr_ip);
    case AF_INET6:return BindSock6(sfd, port, ifr_ip);
    default:return sfd;
  }
}
...
int Listen(const uint16_t port, const char *local_ip, int back_log) {
  ...
  if (listen(fd, back_log) == -1) {
    RD_WARN("Listen socket failed: {}", GetErrorMsg(true));
    close(fd);
    return -1;
  }
...
bool GetDomainIP(const char *host, uint16_t port, struct sockaddr_storage &addr,
  ...
  if (flag) {
    switch (addr.ss_family) {
      case AF_INET :((sockaddr_in *) &addr)->sin_port = htons(port);
        break;
      case AF_INET6 :((sockaddr_in6 *) &addr)->sin6_port = htons(port);
        break;
      default:break;
    }
  }
