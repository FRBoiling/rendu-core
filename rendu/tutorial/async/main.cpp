...
void tutorial_threadpool_1(ThreadPoolPtr tp) {
  /**
     * 依次向线程池中传入：
     * 1、普通函数
     * 2、静态函数
...
void tutorial_threadpool_2(ThreadPoolPtr tp) {
  /**
  ...
  /** 添加一个不耗时的任务 */
  int i = 1, j = 2, k = 3;
  auto hcg = [] { RD_ECHO("Hello, CGraph."); };
  taskGroup.addTask(hcg);

  /** 添加一个耗时为1000ms的任务 */
  taskGroup.addTask([i, j] {
    int result = i + j;
    RD_SLEEP_MILLISECOND(1000)
    RD_ECHO("sleep for 1 second, [%d] + [%d] = [%d], run success.", i, j, result);
  });

  taskGroup.addTask([i, j, k] {
    int result = i - j + k;
    RD_SLEEP_MILLISECOND(2000)
    RD_ECHO("sleep for 2 second, [%d] - [%d] + [%d] = [%d], run success.", i, j, k, result);
    return result;// submit接口，不会对线程函数返回值进行判断。如果需要判断，考虑commit方式
  });

  //  /** 如果添加耗时3000ms的任务，则整体执行失败 */
  //   taskGroup.addTask([] {
  //          RD_SLEEP_MILLISECOND(3000)
  //     });
  ...
  /**
     ...
     * */
  taskGroup.setOnFinished([](const Status &sts) {
    if (sts.isOK()) {
      RD_ECHO("task group run success.");
    } else {
      RD_ECHO("task group run failed, error info is [%s].", sts.getInfo().c_str());
    }
  });
  ...
  RD_ECHO("task group run status is [%d].", status.getCode());
}


void tutorial_threadpool_3(ThreadPoolPtr tp) {
  /**
     * 并发打印0~100之间的数字
     ...
     */
  const int size = 100;
  RD_ECHO("thread pool task submit version : ");
  for (int i = 0; i < size; i++) {
    tp->submit([i] { std::cout << i << " "; });// 可以看到，submit版本是有序执行的。如果需要想要无序执行，可以通过创建taskGroup的方式进行，或者使用commit方法
  }
  RD_SLEEP_SECOND(1)// 等待上面函数执行完毕，以便于观察结果。无实际意义
  std::cout << "\r\n";

  RD_ECHO("thread pool task group submit version : ");
  TaskGroup taskGroup;
  for (int i = 0; i < size; i++) {
  ...
  RD_ECHO("thread pool task commit version : ");
  for (int i = 0; i < size; i++) {
...
int main() {
  ...
  RD_ECHO("======== tutorial_threadpool_1 begin. ========");
  tutorial_threadpool_1(pool.get());

  RD_ECHO("======== tutorial_threadpool_2 begin. ========");
  tutorial_threadpool_2(pool.get());

  RD_ECHO("======== tutorial_threadpool_3 begin. ========");
