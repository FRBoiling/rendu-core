...
void ConsoleChannel::Format(const ALogger &logger, std::ostream &ost, const LogContext::Ptr &ctx) {
  if (enable_color_) {
    // color console start
#ifdef _WIN32
...
void ConsoleChannel::Write(const LogContext::Ptr &ctx, ALogger &logger) {
