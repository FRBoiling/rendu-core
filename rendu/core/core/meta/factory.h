...
#ifndef RENDU_CORE_META_FACTORY_H_
#define RENDU_CORE_META_FACTORY_H_
...
[[nodiscard]] inline decltype(auto) owner(meta_ctx &ctx, const type_info &info) {
  ...
  RD_ASSERT(context.value.contains(info.hash()), "Type not available");
...
class meta_factory {
 ...
 public:
  ...
  auto type(const id_type id) noexcept {
    auto &&elem = internal::owner(*ctx, *info);
    RD_ASSERT(elem.id == id || !resolve(*ctx, id), "Duplicate identifier");
    bucket = &elem.details->prop;
    elem.id = id;
    return *this;
  }
  ...
  meta_factory prop(id_type id, [[maybe_unused]] Value &&...value) {
    RD_ASSERT(bucket != nullptr, "Meta object does not support properties");
...
#endif //RENDU_CORE_META_FACTORY_H_
