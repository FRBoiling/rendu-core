...
#ifndef RENDU_CORE_CORE_ECS_VIEW_H_
#define RENDU_CORE_CORE_ECS_VIEW_H_
#include <array>
#include <iterator>
#include <tuple>
#include <type_traits>
...
[[nodiscard]] auto none_of(const std::array<const Type *, N> &filter, const typename Type::entity_type entity) noexcept {
  return std::apply([entity](const auto *...curr) { return (!curr->contains(entity) && ...); }, filter);
}
...
struct extended_view_iterator final {
  ...
  [[nodiscard]] reference operator*() const noexcept {
    return std::apply([entity = *it](auto *...curr) { return std::tuple_cat(std::make_tuple(entity), curr->get_as_tuple(entity)...); }, pools);
  }
...
void each(Func &func, std::index_sequence<Index...>) const {
  for(const auto curr: storage<Curr>().each()) {
    if(const auto entity = std::get<0>(curr); ((sizeof...(Get) != 1u) || (entity != tombstone)) && ((Curr == Index || storage<Index>().contains(entity)) && ...) && internal::none_of(filter, entity)) {
...
[[nodiscard]] bool contains(const entity_type entity) const noexcept {
  return std::apply([entity](const auto *...curr) { return (curr->contains(entity) && ...); }, pools) && internal::none_of(filter, entity);
}
...
[[nodiscard]] decltype(auto) get(const entity_type entity) const {
  if constexpr(sizeof...(Type) == 0) {
    ...
    return (storage<index_of<Type>>().get(entity), ...);
  } else {
    return std::tuple_cat(storage<index_of<Type>>().get_as_tuple(entity)...);
  }
}
...
#endif //RENDU_CORE_CORE_ECS_VIEW_H_
