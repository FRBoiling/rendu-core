...
namespace detail {
  struct cancellation_registration_list_chunk {
    ...
    std::atomic<CancellationTokenRegistration *> m_entries[1];
  };
...
detail::cancellation_registration_list_chunk *
detail::cancellation_registration_list_chunk::allocate(std::uint32_t entryCount) {
  ...
  for (std::uint32_t i = 0; i < entryCount; ++i) {
    ::new (&chunk->m_entries[i]) std::atomic<CancellationTokenRegistration *>(nullptr);
  }

  return chunk;
}

void detail::cancellation_registration_list_chunk::free(
    cancellation_registration_list_chunk *chunk) noexcept {
  std::free(chunk);
}

detail::cancellation_registration_list *
detail::cancellation_registration_list::allocate() {
  ...
  for (std::uint32_t i = 0; i < initialChunkSize; ++i) {
    ::new (&bucket->m_headChunk.m_entries[i]) std::atomic<CancellationTokenRegistration *>(nullptr);
  }

  return bucket;
}

void detail::cancellation_registration_list::free(cancellation_registration_list *list) noexcept {
  std::free(list);
}

detail::cancellation_registration_state *
detail::cancellation_registration_state::allocate() {
...
void detail::cancellation_registration_state::free(cancellation_registration_state *state) noexcept {
  std::free(state);
}

detail::cancellation_registration_result
detail::cancellation_registration_state::add_registration(
    CancellationTokenRegistration *registration) {
  ...
  if (list == nullptr) {
    ...
    ::new (&newList->m_headChunk.m_entries[0]) std::atomic<CancellationTokenRegistration *>(registration);
  ...
  while (true) {
    ...
    const std::uint32_t elementCount =
        lastChunk->m_entryCount < maxElementCount ? lastChunk->m_entryCount * 2 : maxElementCount;
    ...
    registration->m_entryIndex = 0;
    ::new (&newChunk->m_entries[0]) std::atomic<CancellationTokenRegistration *>(registration);
...
detail::cancellation_state *detail::cancellation_state::create() {
  return new cancellation_state();
}

detail::cancellation_state::~cancellation_state() {
...
void detail::cancellation_state::add_token_ref() noexcept {
  m_state.fetch_add(cancellation_token_ref_increment, std::memory_order_relaxed);
}

void detail::cancellation_state::release_token_ref() noexcept {
...
void detail::cancellation_state::add_source_ref() noexcept {
  m_state.fetch_add(cancellation_source_ref_increment, std::memory_order_relaxed);
}

void detail::cancellation_state::release_source_ref() noexcept {
...
bool detail::cancellation_state::can_be_cancelled() const noexcept {
  return (m_state.load(std::memory_order_acquire) & can_be_cancelled_mask) != 0;
}

bool detail::cancellation_state::is_cancellation_requested() const noexcept {
  return (m_state.load(std::memory_order_acquire) & cancellation_requested_flag) != 0;
}

bool detail::cancellation_state::is_cancellation_notification_complete() const noexcept {
  return (m_state.load(std::memory_order_acquire) & cancellation_notification_complete_flag) != 0;
}

void detail::cancellation_state::request_cancellation() {
...
bool detail::cancellation_state::try_register_callback(
    CancellationTokenRegistration *registration) {
  ...
  if ((m_state.load(std::memory_order_seq_cst) & cancellation_requested_flag) != 0) {
    ...
    // no other thread will have written to the CancellationTokenRegistration record
...
void detail::cancellation_state::deregister_callback(CancellationTokenRegistration *registration) noexcept {
...
detail::cancellation_state::cancellation_state() noexcept
    : m_state(cancellation_source_ref_increment), m_registrationState(nullptr) {
}
