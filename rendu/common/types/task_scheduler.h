#ifndef RENDU_TYPES_TASKSCHEDULER_H_
#define RENDU_TYPES_TASKSCHEDULER_H_
...
namespace rendu::types
{
    ...
    public:
        ...
        {
            RENDU_ASSERT(repeat, "Repeat is null");
            if(!repeat) {
                RENDU_MSG_ERROR("TaskID: {} not created, repeat is null!", id);
                return;
            }
            ...
            RENDU_ASSERT(result);
            if(!result) {
                RENDU_MSG_ERROR("TaskID: {} already exist!", id);
                return;
            }
        ...
        void Remove(std::uint64_t id)
        {
            if(const auto it = m_Tasks.find(id); it != m_Tasks.cend())
            {
                ...
                RENDU_ASSERT(itr != m_Times.end(), "WTF? Why not found?");
                if(itr != m_Times.end()) {
                    m_Times.erase(itr);
                }
                m_Tasks.erase(it);
        ...
        void Update()
        {
            while(!m_Times.empty())
            {
                ...
                RENDU_ASSERT(task->m_Repeat, "WTF? Repeat is null");
                ...
                task = it->m_Task;
                RENDU_ASSERT(task, "WTF? Task is null");
                if(task->m_Repeat)
                {
...
#endif // RENDU_TYPES_TASKSCHEDULER_H_