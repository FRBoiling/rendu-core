#ifndef RENDU_TYPES_ALIGNED_STORAGE_H_
#define RENDU_TYPES_ALIGNED_STORAGE_H_
...
namespace rendu::types
{
    ...
    private:
        ...
        static void Create(Storage<T>& storage, std::size_t index, Args&&... args)
        {
            ...
            if constexpr(std::is_array_v<T>) {
                RENDU_ASSERT(index < std::extent_v<T>, "Out of bounds!");
            }
    ...
    public:
        ...
        static void Construct(Storage<T>& storage, std::size_t pos, std::size_t size)
            ...
            RENDU_ASSERT((pos + size) <= std::extent_v<T>, "Out of bounds!");
        ...
        static void ConstructCopy(const Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to)
        ...
        template <typename T>
        requires std::conjunction_v<std::is_array<T>, std::is_copy_constructible<std::remove_all_extents_t<T>>>
        static void ConstructCopy(const Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to, std::size_t index)
            noexcept(std::is_nothrow_copy_constructible_v<std::remove_extent_t<T>>) {
            RENDU_ASSERT(index < std::extent_v<T>, "Out of bounds!");
        ...
        template <typename T>
        requires std::conjunction_v<std::is_array<T>, std::is_copy_constructible<std::remove_all_extents_t<T>>>
        static void ConstructCopy(const Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to, std::size_t pos, std::size_t size)
            noexcept(std::is_nothrow_copy_constructible_v<std::remove_extent_t<T>>) {
            RENDU_ASSERT((pos + size) <= std::extent_v<T>, "Out of bounds!");
        ...
        static void ConstructMove(Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to)
        ...
        template <typename T>
        requires std::conjunction_v<std::is_array<T>, std::is_move_constructible<std::remove_extent_t<T>>>
        static void ConstructMove(Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to, std::size_t index)
            noexcept(std::is_nothrow_move_constructible_v<std::remove_extent_t<T>>) {
            RENDU_ASSERT(index < std::extent_v<T>, "Out of bounds!");
        ...
        template <typename T>
        requires std::conjunction_v<std::is_array<T>, std::is_move_constructible<std::remove_extent_t<T>>>
        static void ConstructMove(Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to, std::size_t pos, std::size_t size)
            noexcept(std::is_nothrow_move_constructible_v<std::remove_extent_t<T>>) {
            RENDU_ASSERT((pos + size) <= std::extent_v<T>, "Out of bounds!");
        ...
        static void OperatorCopy(const Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to)
        ...
        template <typename T>
        requires std::conjunction_v<std::is_array<T>, std::is_copy_assignable<std::remove_all_extents_t<T>>>
        static void OperatorCopy(const Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to, std::size_t index)
            noexcept(std::is_nothrow_copy_assignable_v<std::remove_extent_t<T>>) {
            RENDU_ASSERT(index < std::extent_v<T>, "Out of bounds!");
        ...
        template <typename T>
        requires std::conjunction_v<std::is_array<T>, std::is_copy_assignable<std::remove_all_extents_t<T>>>
        static void OperatorCopy(const Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to, std::size_t pos, std::size_t size)
            noexcept(std::is_nothrow_copy_assignable_v<std::remove_extent_t<T>>) {
            RENDU_ASSERT((pos + size) <= std::extent_v<T>, "Out of bounds!");
        ...
        static void OperatorMove(Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to)
        ...
        template <typename T>
        requires std::conjunction_v<std::is_array<T>, std::is_move_assignable<std::remove_extent_t<T>>>
        static void OperatorMove(Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to, std::size_t index)
            noexcept(std::is_nothrow_move_assignable_v<std::remove_extent_t<T>>) {
            RENDU_ASSERT(index < std::extent_v<T>, "Out of bounds!");
        ...
        template <typename T>
        requires std::conjunction_v<std::is_array<T>, std::is_move_assignable<std::remove_extent_t<T>>>
        static void OperatorMove(Storage<T>& RENDU_RESTRICT from, Storage<T>& RENDU_RESTRICT to, std::size_t pos, std::size_t size)
            noexcept(std::is_nothrow_move_assignable_v<std::remove_extent_t<T>>) {
            RENDU_ASSERT((pos + size) <= std::extent_v<T>, "Out of bounds!");
        ...
        static void Copy(Storage<T>& storage, std::size_t from, std::size_t to, std::size_t size)
            noexcept(std::is_nothrow_copy_assignable_v<std::remove_extent_t<T>>) {
            RENDU_ASSERT((from + size) <= std::extent_v<T> && (to + size) <= std::extent_v<T>, "Out of bounds!");
...
#endif // RENDU_TYPES_ALIGNED_STORAGE_H_