...
//    DateTime::DateTime() : m_timeZ(date::current_zone()->name()), m_timeT(0) {
    ...
    DateTime::DateTime(INT64 timeS, STRING timeZ) : m_timeStamp(timeS), m_timeZone(std::move(timeZ)) {
    }
    ...
    STRING DateTime::ToString(const STRING &timeZone) const {
//      std::chrono::system_clock::time_point timePoint = std::chrono::system_clock::from_time_t(m_timeStamp);
    ...
    DateTime DateTime::GetCurrentDateTime(STRING timeZ) {
...
//    DateTime& DateTime::ConvertToTimeZone(const STRING &timeZone) const {
