...
- [x] 故障转移 (FailoverManager)
  ...
  - 自定义健康检查器
  - 手动健康检查
- [x] Prometheus 指标导出 (PrometheusExporter)
  - HTTP 服务器监听
  - /metrics 端点
  - 配置支持 (地址、端口、端点路径)
  - 启动/停止控制
...
#### ActorRef 序列化实现
- ✅ serialize() 序列化方法（path:id 格式）
- ✅ deserialize() 反序列化方法（完整验证）
- ✅ is_valid_serialized() 验证函数
- ✅ 特殊字符处理（空路径、冒号、换行等）
- ✅ 错误处理（格式错误、无效ID）
- ✅ 相等性测试（序列化后比较）
- ✅ 批量序列化测试
- ✅ to_string vs serialize 区别
- ✅ 空路径和纯数字ID验证
- ✅ 单元测试完成（7个测试用例, 55个断言）
  - 基础序列化测试
  - 特殊字符序列化测试
...
- ✅ Config 配置
  - listen_address: 监听地址
  - listen_port: 监听端口
  - metrics_endpoint: 指标端点路径
  - thread_pool_size: 线程池大小
- ✅ HTTP 服务器（简化实现）
- ✅ 启动/停止控制
- ✅ 配置接口（get_port, get_address）
- ✅ MetricsCollector 集成
- ✅ 线程安全
- ✅ 单元测试完成（5个测试用例, 18个断言）
  - 基本构造测试（默认配置、自定义配置）
  - 启动停止测试（正常启停、重复启动、重复停止）
  - 配合 MetricsCollector 测试（Counter, Gauge, Histogram）
  - 多个指标类型混合导出测试
  - 线程安全测试（并发导出指标）
