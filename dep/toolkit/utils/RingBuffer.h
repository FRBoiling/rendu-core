...
private:
    ...
    std::shared_ptr<RingReader> attach(const EventLoop::Ptr &poller, bool use_cache) {
...
public:
    ...
    std::shared_ptr<RingReader> attach(const EventLoop::Ptr &poller, bool use_cache = true) {
    ...
    void getInfoList(const onGetInfoCB &cb, const typename RingReaderDispatcher::onChangeInfoCB &on_change = nullptr) {
        ...
        // 1、最少确保一个元素
        info_vec->resize(_dispatcher_map.IsEmpty() ? 1 : _dispatcher_map.Size());
        ...
        for (auto &pr : _dispatcher_map) {
            auto &second = pr.second;
          pr.first->Async(
            [second, info_vec, on_finished, i, on_change]() { (*info_vec)[i] = second->getInfoList(on_change); });
            ++i;
        }
    }

private:
    void onSizeChanged(const EventLoop::Ptr &poller, int size, bool add_flag) {
        if (size == 0) {
            LOCK_GUARD(_mtx_map);
...
private:
    ...
    std::unordered_map<EventLoop::Ptr, typename RingReaderDispatcher::Ptr, HashOfPtr> _dispatcher_map;
};
