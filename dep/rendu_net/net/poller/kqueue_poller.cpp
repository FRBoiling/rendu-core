...
//  }
//
//  KqueuePoller::KqueuePoller(NetEventLoop* loop)
//    : NetPoller(loop),
//      poller_fd_(-1),
//      events_(nullptr),
//      eventsMask_(nullptr) {
...
//      assert(0);
//    }
//    SocketHelper::SetCloExec(poller_fd_);
...
//  }
//
//  void KqueuePoller::AddEvent(NetChannel *channel, int mask) const {
...
//        LOG_CRITICAL << "kevent op =" << operationToString(EV_ADD) << " fd =" << fd;
//      }
//    }
//    if (mask & RD_WRITABLE) {
...
//  }
//
//  void KqueuePoller::DelEvent(NetChannel *channel, int mask) const {
//    Socket::Ptr socket = channel->GetSocket();
//    SOCKET fd = socket->GetFD();
//    struct kevent event;
//    if (mask & RD_READABLE) {
//      EV_SET(&event, fd, EVFILT_READ, EV_DELETE, 0, 0, NULL);
//      if (kevent(poller_fd_, &event, 1, nullptr, 0, nullptr) == -1) {
//        LOG_CRITICAL << "epoll_ctl op =" << operationToString(EV_DELETE) << " fd =" << fd;
//      }
//    }
//    if (mask & RD_WRITABLE) {
...
//  }
//
//  Timestamp KqueuePoller::Poll(int timeoutMs, NetPoller::NetChannelList &activeChannels) {
...
//          auto* channel = static_cast<NetChannel*>(event.udata);
