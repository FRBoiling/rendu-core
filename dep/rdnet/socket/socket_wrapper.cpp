...
RD_NAMESPACE_BEGIN
  ...
  void SocketWrapper::setPoller(const EventPoller::Ptr &poller) {
    _poller = poller;
  }
  ...
  const EventPoller::Ptr &SocketWrapper::getPoller() const {
    assert(_poller);
    return _poller;
  }
  ...
  EndPoint *SocketWrapper::GetEndPoint() {
    return nullptr;
  }

  void SocketWrapper::Bind(EndPoint *endPoint) {
