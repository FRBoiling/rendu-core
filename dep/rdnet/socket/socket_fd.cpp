...
RD_NAMESPACE_BEGIN
  ...
  SocketFD::SocketFD(int num, SocketNum::SockType type, const EventPoller::Ptr &poller) {
    _num = std::make_shared<SocketNum>(num, type);
    _poller = poller;
  }

  SocketFD::SocketFD(const SocketFD &that, const EventPoller::Ptr &poller) {
    _num = that._num;
    _poller = poller;
  ...
  const EventPoller::Ptr &SocketFD::GetPoller() const {
    return _poller;
  }
