...
RD_NAMESPACE_BEGIN
  ...
  int SockUtil::bind_sock6(int fd, const char *local_ip, uint16_t local_port) {
    struct sockaddr_in6 addr;
    bzero(&addr, sizeof(addr));
    addr.sin6_family = AF_INET6;
    addr.sin6_port = htons(local_port);
    if (1 != inet_pton(AF_INET6, local_ip, &(addr.sin6_addr))) {
      if (strcmp(local_ip, "0.0.0.0")) {
//        WarnL << "inet_pton to ipv6 address failed: " << ifr_ip;
      }
      addr.sin6_addr = IN6ADDR_ANY_INIT;
    }
    if (::bind(fd, (struct sockaddr *) &addr, sizeof(addr)) == -1) {
//      WarnL << "Bind socket failed: " << get_rd_errmsg(true);
      return -1;
    }
    return 0;
  }

  int SockUtil::bind_sock4(int fd, const char *local_ip, uint16_t local_port) {
    struct sockaddr_in addr;
    bzero(&addr, sizeof(addr));

    addr.sin_family = AF_INET;
    addr.sin_port = htons(local_port);
    if (1 != inet_pton(AF_INET, local_ip, &(addr.sin_addr))) {
      if (strcmp(local_ip, "::")) {
        WarnL << "inet_pton to ipv4 GetAddress failed: " << local_ip;
      }
  ...
  void SockUtil::Close(const int32 sfd) {
    close(sfd);
  }

  SOCKET SockUtil::Bind(const char *ifr_ip, uint16_t port, int family) {
    ...
    switch (family) {
      case AF_INET:
        return bind_sock4(sfd, ifr_ip, port);
      case AF_INET6:
        return bind_sock6(sfd, ifr_ip, port);
      default:
        return sfd;
    }
  }

  void SockUtil::Listen(const SOCKET sfd, int back_log /*= 1024*/) {
    //开始监听
    if (listen(sfd, back_log) == -1) {
//      WarnL << "Listen socket failed: " << get_rd_errmsg(true);
      Close(sfd);
    }
  }

  SOCKET SockUtil::Accept(const SOCKET listen_sfd) {
    SOCKET sockfd = -1;
...
//  SOCKET SockUtil::Connect(const char *remote_host, uint16_t remote_port) {
//    SOCKET sockfd = -1;
//    struct sockaddr_storage storage;
...
#define TCP_KEEPIDLE TCP_KEEPALIVE
  ...
  private:
    ...
    std::shared_ptr<struct addrinfo> getSystemDomainIP(const char *host) {
      ...
      } while (ret == -1 && get_rd_error(true) == RD_EINTR);
  ...
  static int bind_sock6(int fd, const char *ifr_ip, uint16_t port) {
    ...
    if (1 != inet_pton(AF_INET6, ifr_ip, &(addr.sin6_addr))) {
      if (strcmp(ifr_ip, "0.0.0.0")) {
        WarnL << "inet_pton to ipv6 GetAddress failed: " << ifr_ip;
      }
      addr.sin6_addr = IN6ADDR_ANY_INIT;
    }
    if (::bind(fd, (struct sockaddr *) &addr, sizeof(addr)) == -1) {
  ...
  static int bind_sock4(int fd, const char *ifr_ip, uint16_t port) {
    ...
    if (1 != inet_pton(AF_INET, ifr_ip, &(addr.sin_addr))) {
      if (strcmp(ifr_ip, "::")) {
        WarnL << "inet_pton to ipv4 GetAddress failed: " << ifr_ip;
      }
      addr.sin_addr.s_addr = INADDR_ANY;
    }
    if (::bind(fd, (struct sockaddr *) &addr, sizeof(addr)) == -1) {
  ...
  int SockUtil::Connect(const char *host, uint16_t port, bool async, const char *local_ip, uint16_t local_port) {
    ...
    if (async && get_rd_error(true) == RD_EAGAIN) {
      //异步连接成功
      return sockfd;
    }
    WarnL << "Connect socket to " << host << " " << port << " failed: " << get_rd_errmsg(true);
    close(sockfd);
    return -1;
  }
  ...
  int SockUtil::Listen(const uint16_t port, const char *local_ip, int back_log) {
    ...
    if ((fd = (int) socket(family, SOCK_STREAM, IPPROTO_TCP)) == -1) {
      WarnL << "Create socket failed: " << get_rd_errmsg(true);
      return -1;
    }
  ...
  int SockUtil::getSockError(int fd) {
    ...
    if (getsockopt(fd, SOL_SOCKET, SO_ERROR, (char *) &opt, &optLen) < 0) {
      return get_rd_error(true);
    } else {
      return rd_translate_posix_error(opt);
    }
  }
...
void for_each_netAdapter_posix(FUN &&fun){ //type: struct ifreq *
    ...
    if (-1 == ioctl(sockfd, SIOCGIFCONF, &ifconf)) {    //获取所有接口信息
        WarnL << "ioctl SIOCGIFCONF failed: " << get_rd_errmsg(true);
        close(sockfd);
        return;
    }
    close(sockfd);
  ...
  int SockUtil::DissolveUdpSock(int fd) {
    ...
    if (-1 == ::connect(fd, (struct sockaddr *) &addr, addr_len) && get_rd_error() != RD_EAFNOSUPPORT) {
      // mac/ios时返回EAFNOSUPPORT错误
      WarnL << "Connect socket AF_UNSPEC failed: " << get_rd_errmsg(true);
      return -1;
    }
    return 0;
  }
  ...
  string SockUtil::get_ifr_mask(const char *if_name) {
    ...
    if ((ioctl(fd, SIOCGIFNETMASK, &ifr_mask)) < 0) {
        WarnL << "ioctl SIOCGIFNETMASK on " << if_name << " failed: " << get_rd_errmsg(true);
        close(fd);
        return "";
    }
    close(fd);
  ...
  string SockUtil::get_ifr_brdaddr(const char *if_name) {
    ...
    if ((ioctl(fd, SIOCGIFBRDADDR, &ifr_mask)) < 0) {
        WarnL << "ioctl SIOCGIFBRDADDR failed: " << get_rd_errmsg(true);
        close(fd);
        return "";
    }
    close(fd);
...
#define ip_addr_netcmp(addr1, addr2, mask) (((addr1) & (mask)) == ((addr2) & (mask)))
  ...
  int SockUtil::joinMultiAddr(int fd, const char *addr, const char *local_ip) {
    ...
    if (ret == -1) {
      TraceL << "setsockopt IP_ADD_MEMBERSHIP failed: " << get_rd_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
  ...
  int SockUtil::leaveMultiAddr(int fd, const char *addr, const char *local_ip) {
    ...
    if (ret == -1) {
      TraceL << "setsockopt IP_DROP_MEMBERSHIP failed: " << get_rd_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
  ...
  int SockUtil::joinMultiAddrFilter(int fd, const char *addr, const char *src_ip, const char *local_ip) {
    ...
    if (ret == -1) {
      TraceL << "setsockopt IP_ADD_SOURCE_MEMBERSHIP failed: " << get_rd_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
  ...
  int SockUtil::leaveMultiAddrFilter(int fd, const char *addr, const char *src_ip, const char *local_ip) {
    ...
    if (ret == -1) {
      TraceL << "setsockopt IP_DROP_SOURCE_MEMBERSHIP failed: " << get_rd_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
