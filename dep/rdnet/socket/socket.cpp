...
#include "buffer/buffer_socket.h"
...
#define LOCK_GUARD(mtx) std::lock_guard<decltype(mtx)> lck(mtx)
  ...
  void Socket::Bind(EndPoint *endPoint) {
    ...
    if (m_ipEndpoint->IsIPv4()) {
      m_sfd = SockUtil::Bind(m_ipEndpoint->GetAddress().ToString().c_str(), m_ipEndpoint->GetPort(), AF_INET);
    } else {
      m_sfd = SockUtil::Bind(m_ipEndpoint->GetAddress().ToString().c_str(), m_ipEndpoint->GetPort(), AF_INET6);
    }
  }

  void Socket::Listen(int backlog) {
    SockUtil::Listen(m_sfd, backlog);
  }

  ISocket *Socket::Accept() {
    int fd = SockUtil::Accept(m_sfd);
  ...
  void Socket::Close() {
    SockUtil::Close(m_sfd);
  }

  void Socket::Connect(const char *remote_host, uint16_t remote_port) {
    m_sfd = SockUtil::Connect(remote_host, remote_port);
  ...
  void Socket::setOnFlush(onFlush cb) {
    ...
    if (cb) {
      ...
      _on_flush = []() {
        WarnL << "Socket not set flush callback";
        return true;
      };
    }
  }
...
#define CLOSE_SOCK(fd) if (fd != -1) { close(fd);}
  ...
  void Socket::connect_l(const string &url, uint16_t port, const onErrCB &con_cb_in, float timeout_sec,
      ...
      ([weak_self, con_cb](int sock) {
        ...
        if (sock == -1 || !strong_self) {
          if (!strong_self) {
            ...
            con_cb(SockException(Err_dns, get_rd_errmsg(true)));
          }
          return;
        }

        // 监听该socket是否可写，可写表明已经连接服务器成功
        int result = strong_self->_poller->AddEvent(sock, EventPoller::Event_Write,
    ...
    _con_timer = std::make_shared<Timer>(timeout_sec, [weak_self, con_cb]() {
      con_cb(SockException(Err_timeout, rd_strerror(RD_ETIMEDOUT)));
      return false;
    }, _poller);
  ...
  bool Socket::attachEvent(int sock, SocketNum::SockType type) {
    ...
    if (type == SocketNum::Sock_TCP_Server) {
      // tcp服务器
      auto result = _poller->AddEvent(sock, EventPoller::Event_Read | EventPoller::Event_Error,
    ...
    auto result = _poller->AddEvent(sock, EventPoller::Event_Read | EventPoller::Event_Error | EventPoller::Event_Write,
  ...
  ssize_t Socket::onRead(int sock_fd, SocketNum::SockType type, const BufferRaw::Ptr &buffer) noexcept {
    ...
    while (_enable_recv) {
      ...
      } while (-1 == nread && RD_EINTR == get_rd_error(true));
      ...
      if (nread == -1) {
        auto err = get_rd_error(true);
        if (err != RD_EAGAIN) {
          if (type == SocketNum::Sock_TCP) {
            emitErr(ToSockException(err));
          } else {
            WarnL << "Recv err on udp socket[" << sock_fd << "]: " << rd_strerror(err);
          }
        }
        return ret;
      }
  ...
  int Socket::onAccept(int sock, int event) noexcept {
    ...
    while (true) {
      if (event & EventPoller::Event_Read) {
        ...
        } while (-1 == fd && RD_EINTR == get_rd_error(true));

        if (fd == -1) {
          int err = get_rd_error(true);
          if (err == RD_EAGAIN) {
            // 没有新连接
            return 0;
          }
  ...
  bool Socket::flushData(int sock, SocketNum::SockType type, bool poller_thread) {
    ...
    while (!send_buf_sending_tmp.empty()) {
      ...
      if (type == SocketNum::Sock_UDP) {
        // udp发送异常，把数据丢弃
        send_buf_sending_tmp.pop_front();
        WarnL << "Send udp socket[" << sock << "] failed, Data ignored: " << rd_strerror(err);
        continue;
      }
      // tcp发送失败时，触发异常
      emitErr(ToSockException(err));
  ...
  bool Socket::bindPeerAddr(const struct sockaddr *dst_addr, socklen_t addr_len, bool soft_bind) {
    ...
    if (soft_bind) {
      ...
      if (-1 == ::connect(_sock_fd->RawFd(), dst_addr, addr_len)) {
        WarnL << "Connect socket to peer GetAddress failed: " << SockUtil::inet_ntoa(dst_addr);
        return false;
      }
