...
#ifndef RENDU_SOCKET_UTIL_H
#define RENDU_SOCKET_UTIL_H
...
//套接字工具类，封装了socket、网络的一些基本操作
  ...
  public:
    ...
    */
    static SOCKET Bind(const char *local_ip, uint16_t local_port, int family);
    ...
    */
    static void Listen(SOCKET sfd, int back_log = 1024);

    /**
    * 监听套接字
    * @param listen_sfd 套接字文件描述符
    * @return -1代表失败，其他为socket fd号
    */
    static SOCKET Accept(const SOCKET listen_sfd);

    /**
    * 关闭套接字
    * @param sfd 套接字文件描述符
    */
    static void Close(SOCKET sfd);
  ...
  public:
    /**
        ...
        * @return -1代表失败，其他为socket fd号
        */
    static SOCKET Connect(const char *host, uint16_t port, bool async = true, const char *local_ip = "::", uint16_t local_port = 0);
  ...
  private:
    static int bind_sock4(int fd, const char *local_ip, uint16_t local_port);

    static int bind_sock6(int fd, const char *local_ip, uint16_t local_port);
  };

RD_NAMESPACE_END
#endif //RENDU_SOCKET_UTIL_H
