...
RD_NAMESPACE_BEGIN
  ...
  TcpServer::TcpServer(const EventPoller::Ptr &poller) : AService(poller) {
  ...
  TcpServer::~TcpServer() {
    if (_main_server && _socket && _socket->rawFD() != -1) {
      InfoL << "Close tcp server [" << _socket->GetLocalIp() << "]: " << _socket->GetLocalPort();
    }
    _timer.reset();
...
// 接收到客户端连接请求
  ...
  void TcpServer::listen(uint16_t port, const std::string &host, uint32_t backlog) {
    ...
    if (!_socket->listen(port, host.c_str(), backlog)) {
      ...
      string err = (StrPrinter << "Listen on " << host << " " << port << " failed: " << get_rd_errmsg(true));
