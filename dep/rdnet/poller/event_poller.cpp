...
RD_NAMESPACE_BEGIN
  ...
  int EventPoller::RemoveEvent(int fd, PollDelCallBack callback) {
    TimeTicker();
    if (!callback) {
  ...
  inline void EventPoller::onPipeEvent() {
    ...
    while (true) {
      ...
      if (err == 0 || get_rd_error(true) != RD_EAGAIN) {
...
// static
  ...
  void EventPoller::runLoop(bool blocked, bool ref_self) {
    if (blocked) {
      ...
      _loop_thread = new std::thread(&EventPoller::runLoop, this, true, ref_self);
